	...
	if (ec == rai::error_cli::unknown_command)
	{
		...
		else if (vm.count ("debug_profile_kdf"))
		{
			...
			for (; true;)
			{
				...
				argon2_hash (1, rai::wallet_store::kdf_work, 1, password.data (), password.size (), salt.bytes.data (), salt.bytes.size (), result.bytes.data (), result.bytes.size (), NULL, 0, Argon2_d, 0x10);
